...
class DataEntryGUI:
    ...
    def create_input_fields(self):
        ...
        self.entry1 = tk.Entry(self.window, width=30).pack(pady=5)

        # Input for second value
        tk.Label(self.window, text="Enter Value 2:").pack(pady=5)
        self.entry2 = tk.Entry(self.window, width=30).pack(pady=5)

        # Input for third value
        tk.Label(self.window, text="Enter Value 3:").pack(pady=5)
        self.entry3 = tk.Entry(self.window, width=30).pack(pady=5)

        # Submit button
        submit_button = tk.Button(self.window, text="Submit", command=self.submit_data).pack(pady=10)

    def submit_data(self):
        """
        Collect data from input fields and call the submit callback.
        """
        input_val1 = self.entry1.get()
        input_val2 = self.entry2.get()
        input_val3 = self.entry3.get()
