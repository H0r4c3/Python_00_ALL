            ...
            for j in range(m):
                if mask[i][j] == '.':
                    if j == 0 or mask[i][j-1] == 'X':
                        word_pos.append((i, j, 'across'))
                    elif i == 0 or mask[i-1][j] == 'X':
                        word_pos.append((i, j, 'down'))
        
        return word_pos, mask
    
    def possible_words(self, word_pos, mask):
        candidates = {}
...
def main():
    ...
    word_pos, mask = crossword.words_positions(mask)
