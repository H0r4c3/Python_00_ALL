...
class CrosswordGrid:
    ...
    def create_buttons(self):
        ...
        # creating a button that will clear the cells
        self.clear_button=tk.Button(self.frame_b, text = 'Clear', command = lambda: self.clear_the_grid())
        ...
        self.fill_button.pack(padx=5, pady=5, side=tk.TOP)
        self.clear_button.pack(padx=5, pady=5, side=tk.TOP)
        self.quit_button.pack(padx=5, pady=5, side=tk.BOTTOM)
        
    def read_cells_from_crossword_grid(self):
        '''
        Compose a new crossword list reading the crossword grid after inserting letters
        '''    
        crossword = [['X' if self.cells_arr[i][j].get() == '$' else self.cells_arr[i][j].get() if self.cells_arr[i][j].get() != '' else '.' for j in range(self.columns)] 
                     for i in range(self.rows)]
        crossword = [''.join(item) for item in crossword]
    ...
    def fill_the_grid(self):
        ...
        '''
        logging.info('Starting "fill_the_grid" method')
        
        from crossword_solver import CrosswordSolver

        config.list_of_words = CrosswordSolver.read_words_from_file(config.my_path)
        
        print(f'The old config.crossword = {config.crossword}')
        config.crossword = self.read_cells_from_crossword_grid()
        print(f'The new config.crossword = {config.crossword}')
        logging.info(f'The new config.crossword = {config.crossword}')
        
        solver = CrosswordSolver(config.crossword, config.list_of_words)
        ...
        logging.debug(f"result = {config.result}")
    ...
    def clear_the_grid(self):
        print(config.crossword)
        for i in range(self.rows):
            for j in range(self.columns):
                    self.cells_arr[i][j].delete(0, tk.END)
                    self.cells_arr[i][j].insert(0, config.crossword[i][j])
